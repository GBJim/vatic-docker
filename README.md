...
* turkic delete ASSIGNMENT  --> Delete a assignment(worker-video pair)
* turkic dump --image  --> Dump video frames into CatlTech Toolkit convention
* turkic dump --json  --> Dump annotations into JSON format. The structure has been modified to frame-based indexing
* turkic dump --split A:B --> Split mechanism that automatically divides data into multiple splits
* turkic dump --vbb   --> Dump annotations into vbb txt files that accepted by CalTech Toolkit
* turkic list --detail  --> List detailed hyperlinks-worker mapping of assignments and generate a user_map file.
