...
function go_seek_frame()
{
  ...
  if (parameters.hasOwnProperty("frame"))
  {
    ...
    player.seek(frame);
    console.log("Player jump to frame:",  frame);

  }
  else{


  console.log("Player has no frame to jump");
}
...
function ui_build(job)
{
    ...
    player = new VideoPlayer(videoframe, job);
...
function go_next_url()
{
  var current_id = mturk_parameters().id;
  next_URL = next_URL[current_id];
  if (next_URL == "end")
  {
      alert("You have went through all video segments!")
  }
  else
  {
...
function go_previous_url()
{
  var current_id = mturk_parameters().id;
  previous_URL = previous_URL[current_id];
  if (previous_URL == "end")
  {
      alert("You are at the first video segment!")
  }
  else
  {
...
function ui_setup(job)
{
    ...
    $("<div class='button' id='previous'>Previous</div>")
        ...
        }).appendTo($("#sidebar")).click(go_previous_url);
    ...
    $("<div class='button' id='next'>Next</div>")
        ...
        }).appendTo($("#sidebar")).click(go_next_url);
